<?php include("header.php");?>
<section class="banner">
   <div class="banner-Container">
      <div class="banner-Container-Item programa">
         <div class="back">
            <img src="img/programacion-1.jpg" alt="">
         </div>
         <div class="content">
            <div class="box">
               <h1>VIVE EL PARTIDASO</h1>
               <div class="text">
                  <h4>27 DE DICIEMBRE</h4>
                  <h5 class="t_green">21:00 H</h5>
                  <h6>La Liga</h6>
                  <a href="" class="btn btn-green">LO QUIERO</a>
               </div>
            </div>
         </div>
      </div>
      <div class="banner-Container-Item programa">
         <div class="back">
            <img src="img/programacion-1.jpg" alt="">
         </div>
         <div class="content">
            <div class="box">
               <h1>VIVE EL PARTIDASO</h1>
               <div class="text">
                  <h4>27 DE DICIEMBRE</h4>
                  <h5 class="t_green">21:00 H</h5>
                  <h6>La Liga</h6>
                  <a href="" class="btn btn-green">LO QUIERO</a>
               </div>
            </div>
         </div>
      </div>
      <div class="banner-Container-Item programa">
         <div class="back">
            <img src="img/alonzo.jpg" alt="">
         </div>
         <div class="content">
            <div class="box">
               <h1>G.P. MALASIA</h1>
               <div class="text">
                  <h4>3 DE AGOSTO</h4>
                  <h5 class="t_green">21:00 H</h5>
                  <h6>La Liga</h6>
                  <a href="" class="btn btn-green">LO QUIERO</a>
               </div>
            </div>
         </div>
      </div>
   </div>
</section>
<section class="content-tab cont-3" style="display: block">
   <img class="back" src="img/rectangles-purple.png" alt="">
   <div class="semanal">
      <div class="content">
         <h2 class="title">PROGRAMACIÓN SEMANAL</h2>
         <div class="content-tab-calendar programacion">
            <div class=" tab-programacion tab_pre_live">
               <p><i class="fa fa-circle" aria-hidden="true"></i>EN DIRECTO</p>
            </div>
            <div class="active tab-programacion tab_pre_semanal">
               <p>PROGRAMACIÓN SEMANAL</p>
            </div>
            <div class="tab-programacion tab_pre_mensual">
               <p>PROGRAMACIÓN MENSUAL</p>
            </div>
         </div>
         <div class="content-tab-calendar tab-live">
            <div class="tab-programacion active" >
               <p>TODOS</p>
            </div>
            <div class="tab-programacion " >
               <p>FÚTBOL</p>
            </div>
            <div class="tab-programacion" >
               <p>MOTOR</p>
            </div>
            <div class="tab-programacion" >
               <p>LUCHA</p>
            </div>
            <div class="tab-programacion" >
               <p>OLYMPIADAS</p>
            </div>
            <div class="tab-programacion" >
               <p>VELA</p>
            </div>
            <div class="tab-programacion" >
               <p>GAMER</p>
            </div>
            <div class="tab-programacion" >
               <p>MAS DÉPORTES</p>
            </div>
         </div>
         <div class="content-tab-calendar tab-semanal">
            <div class="tab-programacion active" data-active="first">
               <p>5 DIC - 11 DIC</p>
            </div>
            <div class="tab-programacion " data-active="second">
               <p>12 DIC - 18 DIC</p>
            </div>
            <div class="tab-programacion" data-active="third">
               <p>19 DIC - 25 DIC</p>
            </div>
            <div class="tab-programacion" data-active="fourth">
               <p>26 DIC - 1 ENE</p>
            </div>
         </div>
         <div class="content-tab-calendar tab-mensual" style="display: none;">
            <div class="tab-programacion active" data-active="month-first">
               <p>DICIEMBRE 2016</p>
            </div>
            <div class="tab-programacion" data-active="month-second">
               <p>ENERO 2017</p>
            </div>
            <div class="tab-programacion" data-active="month-third">
               <p>FEBRERO 2017</p>
            </div>
            <div class="tab-programacion" data-active="month-fourth">
               <p>MARZO 2017</p>
            </div>
         </div>
         <div class="calendar-weekend-container">
            <div class="calendar-weekend first" >
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">5<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">6<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">7<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">8<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">9<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">10<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">11<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>TARDE</p>
                  </div>
                  <div class="segment-tab multiple">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>NOCHE</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
            </div>
            <div class="calendar-weekend second" style="display: none">
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">12<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">13<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">14<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">15<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">16<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">17<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">18<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>TARDE</p>
                  </div>
                  <div class="segment-tab multiple">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>NOCHE</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
            </div>
            <div class="calendar-weekend third" style="display: none">
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">19<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">20<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">21<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">22<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">23<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">24<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">25<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>TARDE</p>
                  </div>
                  <div class="segment-tab multiple">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>NOCHE</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
            </div>
            <div class="calendar-weekend fourth" style="display: none">
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>TARDE</p>
                  </div>
                  <div class="segment-tab multiple">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>NOCHE</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
            </div>
         </div>
         <div class="calendar-month-container" style="display: none">
            <div class="calendar-month month-first">
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
               </div>
            </div>
            <div class="calendar-month month-second" style="display: none">
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
               </div>
            </div>
            <div class="calendar-month month-third" style="display: none">
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
               </div>
            </div>
            <div class="calendar-month month-fourth" style="display: none">
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                     <p>MAÑANA</p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>M <span class="number">27<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>X <span class="number">28<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>J <span class="number">29<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>V <span class="number">30<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>S <span class="number">31<span></p>
                  </div>
                  <div class="segment-tab">
                     <p>D <span class="number">1<span></p>
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="segment-tab">
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
               </div>
               <div class="segment-journe header-segment">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab">
                     <p>L <span class="number">26<span></p>
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
                  <div class="segment-tab">
                  </div>
               </div>
               <div class="segment-journe">
                  <div class="journe-empty">
                  </div>
                  <div class="segment-tab outstanding">
                     <p>NOMBRE DEL EVENTO</p>
                     <p>NOMBRE DEL EVENTO</p>
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
                  <div class="journe-empty">
                  </div>
               </div>
            </div>
         </div>
         <div class="live-calendar-container" style="">
            <div class="live-calendar">
                <div class="time-now">
                    <div class="time"><p>Ahora <span class="time">12:21</span></p></div>
                    <div class="line-across"></div>
                </div>
                <div class="time-line">
                    <div class="line"><i class="fa fa-angle-left"></i><i class="fa fa-angle-right"></i></div>
                </div>
                <div class="time">
                    <div class="hour">
                        <p>00:00</p>
                    </div>
                    <div class="hour">
                        <p>01:00</p>
                    </div>
                    <div class="hour">
                        <p>02:00</p>
                    </div>
                    <div class="hour">
                        <p>03:00</p>
                    </div>
                    <div class="hour">
                        <p>04:00</p>
                    </div>
                    <div class="hour">
                        <p>05:00</p>
                    </div>
                    <div class="hour">
                        <p>06:00</p>
                    </div>
                    <div class="hour">
                        <p>07:00</p>
                    </div>
                    <div class="hour">
                        <p>08:00</p>
                    </div>
                    <div class="hour">
                        <p>09:00</p>
                    </div>
                    <div class="hour">
                        <p>10:00</p>
                    </div>
                    <div class="hour">
                        <p>11:00</p>
                    </div>
                    <div class="hour">
                        <p>13:00</p>
                    </div>
                    <div class="hour">
                        <p>14:00</p>
                    </div>
                    <div class="hour">
                        <p>15:00</p>
                    </div>
                    <div class="hour">
                        <p>16:00</p>
                    </div>
                    <div class="hour">
                        <p>17:00</p>
                    </div>
                    <div class="hour">
                        <p>18:00</p>
                    </div>
                    <div class="hour">
                        <p>19:00</p>
                    </div>
                    <div class="hour">
                        <p>20:00</p>
                    </div>
                    <div class="hour">
                        <p>21:00</p>
                    </div>
                    <div class="hour">
                        <p>22:00</p>
                    </div>
                    <div class="hour">
                        <p>23:00</p>
                    </div>
                    <div class="hour">
                        <p>00:00</p>
                    </div>
                </div>
            </div>
         </div>
         <div class="deportes">
            <h2>¿QUÉ DEPORTES PUEDES VER?</h2>
            <p>Disfruta de todo el deporte en streaming dónde y cuándo quieras... en tu smartTV, consola, PC, Mac, móvil, tableta y
                más dispositivos. Empieza ahora con un mes gratis</p>

                <div class="programas">
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                    <div class="programa-item">
                        <div class="visible-item">
                        <p>FÚTBOL</p>
                        <i class="fa fa-angle-down"></i>
                        </div>
                                        <div class="modal-item">
                            <div class="encabezado"><p>FÚTBOL</p><i class="fa fa-times"></i></div>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                            <a href=""><p>Liga Adelante: F.C. Barcelona - Real Madrid</p></a>
                        </div>

                        
                    </div>
                </div>
         </div>

      </div>
   </div>
   <div class="precont">
      <div class="competencias">
         <h3>Disfruta de todas estas competiciones y mucho más</h3>
         <div class="competiciones">
            <div class="item">
               <img src="img/la-liga.png" alt="">
            </div>
            <div class="item">
               <img src="img/la-liga.png" alt="">
            </div>
            <div class="item">
               <img src="img/la-liga.png" alt="">
            </div>
            <div class="item">
               <img src="img/la-liga.png" alt="">
            </div>
            <div class="item">
               <img src="img/la-liga.png" alt="">
            </div>
            <div class="item">
               <img src="img/la-liga.png" alt="">
            </div>
            <div class="item">
               <img src="img/la-liga.png" alt="">
            </div>
         </div>
         <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce vel massa consectetur, finibus dolor vitae,
            <b>efficitur nisl. Sed sollicitudin velit ut dui ultrices</b> ullamcorper. Cum sociis natoque penatibus et magnis
            dis parturient montes, nascetur ridiculus mus.
         </p>
         <a href="" class="btn btn-purple">CONOCE TODA NUESTRA PROGRAMACIÓN</a>
      </div>
   </div>
</section>
<?php include("footer.php");?>